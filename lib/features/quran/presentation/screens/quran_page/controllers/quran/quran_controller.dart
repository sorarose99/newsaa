...
class QuranController extends GetxController {
  ...
  void onInit() async {
    ...
    state.isPageMode.value = state.box.read(PAGE_MODE) ?? true;
    state.isBold.value = state.box.read(IS_BOLD) ?? false;
  ...
  Future<void> updateTafsir(int pageIndex) async {
    if (state.currentPageNumber.value != pageIndex) {
      ...
      TranslateController.instance.update(['change_tafsir']);
    }
  }
  ...
  void currentListPageNumber() {
    ...
    if (filteredPositions.isNotEmpty) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        final pageOneBased = (firstItemIndex + 1).clamp(1, 604);
