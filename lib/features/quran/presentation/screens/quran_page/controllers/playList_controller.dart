...
class PlayListController extends GetxController {
    ...
    if (firstAyahUQ! <= lastAyahUQ!) {
      ...
      for (int i = firstAyahUQ!; i <= lastAyahUQ!; i++) {
        final surahNum = quranCtrl
            ...
            .padLeft(3, '0');
        String fileName =
            ayahReaderInfo[audioCtrl.state.readerIndex.value]['url'] ==
        ...
        bool downloadResult = await downloadFile(
          generateUrl(i, audioCtrl.state.readerValue!),
          fileName,
        );
  ...
  String generateUrl(int ayahNumber, String readerName) {
    if (ayahReaderInfo[audioCtrl.state.readerIndex.value]['url'] ==
        ApiConstants.ayahs1stSource) {
      log(
        '${ayahReaderInfo[audioCtrl.state.readerIndex.value]['url']}${audioCtrl.reader}/${ayahNumber}.mp3',
      );
      ...
      final currentAyahNumber = quranCtrl
          ...
          .padLeft(3, '0');
      log(
        '${ayahReaderInfo[audioCtrl.state.readerIndex.value]['url']}${audioCtrl.reader}/${surahNum.toString().padLeft(3, "0")}$currentAyahNumber.mp3',
      );
